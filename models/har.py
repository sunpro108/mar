...
class HAR(nn.Module):
    """ Masked Autoencoder with VisionTransformer backbone for harmonization
    """
    ...
    def forward_mae_encoder_no_drop(self, x, x_gray, mask, class_embedding):
        """_summary_
        ...
        # 1. replace the masked region with the same position value in x_gray
        x,x_g = torch.chunk(input=x,chunks=2,dim=0)
        class_embedding,_ = torch.chunk(input=class_embedding,chunks=2,dim=0)
        mask_with_buffer = torch.cat([torch.zeros(x.shape[0], self.buffer_size, device=x.device), mask], dim=1)
        print('after concat buffer mask_with_buffer.shape:', mask_with_buffer.shape)
        ids_masked = mask_with_buffer.nonzero(as_tuple=True)[1]
        x = torch.scatter(x, dim=1, index=ids_masked.unsqueeze(-1).repeat(1,1,embed_dim), src=x_g)
